...
operations of `VcsToolkit.Core` and `VcsToolkit.Forge`. It speaks MCP over stdio, so an agent
harness (an IDE assistant, a coding-agent CLI, or any other MCP client) launches it as a
subprocess and calls its tools instead of shelling out to raw `git`/`jj`/`gh`/`glab`/`tea`
commands.
...
authenticate the ones you intend to use before pointing an agent at the server.
...
| `--allow-tools <name,...>` | a comma-separated list of tool names (repeatable — later occurrences add to the allowed set) | empty (no mutating tool allowed) | Enable only the named mutating tools. Read tools are unaffected — they are always available regardless of this flag. Names are validated up front against the fixed list of mutating tool names (`WriteTools.all`, the same names used for `Destructive`/`ReadOnly` hints below); an unrecognized name is a fatal startup error rather than a silently-inert entry. Ignored if `--allow-write` is also given. |
...
repository's `origin` remote at startup:

- On a **git-backed** repo (or a git-colocated jj repo), it asks `git remote get-url origin`.
- On a **jj-backed** repo without git colocation (`jj git init` without `--colocate`), there is
  no root `.git` for that to find, so it falls back to `jj git remote list
...
`repo_*` mutations, the local-checkout-affecting forge tools
(`forge_pr_checkout`, and `forge_pr_merge`/`forge_pr_close` with `delete_branch`, which can
switch the checkout the same way `repo_checkout` does), and `repo_try_merge` (a real trial merge
that materializes content before rolling itself back, so it needs the same isolation). An MCP
host can dispatch tool calls concurrently; without this lock, two working-copy mutations could
interleave (e.g. a `repo_try_merge` probe's materialize-then-rollback racing a `repo_commit`).
**Remote-only forge writes** (`forge_issue_create`, `forge_issue_close`, `forge_issue_comment`,
`forge_pr_create`, `forge_pr_comment`, `forge_pr_edit`, `forge_pr_mark_ready`, `forge_pr_review`,
and a `forge_pr_merge`/`forge_pr_close` call *without* `delete_branch`) do **not** take this
local lock — they only touch the remote forge, and the forge's own server serializes concurrent
requests on its side.
...
| `repo_annotate` | Per-line authorship of a file at a revision (git blame / jj file annotate), as a JSON array of lines, subject to the same `--output-budget` truncation as `repo_show_file`. `rev` is passed through as-is (git commit-ish or jj revset). | `path` (string, required), `rev` (string, optional — omit to annotate the working copy / `@`) |

#### Writes (gated)

`repo_try_merge` is write-gated but not destructive: it spawns a **real** trial merge that
materializes content in order to detect conflicts, then always rolls itself back — like
`repo_checkout`, materializing content is why it needs write access, even though it leaves no
trace.

| Tool | Purpose | Arguments | Destructive | Idempotent |
|---|---|---|---|---|
| `repo_try_merge` | Probe whether merging `source` into the current work would conflict, without leaving a trace. | `source` (string, required) | no | yes |
...
A tool call fails with one of two MCP error kinds, both carrying a human-readable message:
...
Errors are returned as a normal MCP tool-call error result (`isError: true` with a text content
block carrying the message) — the server process itself keeps running and the MCP session stays
