...
| `forge_pr_list` | Pull/merge requests on the configured forge, open by default and capped at 100 by default. On Gitea, `merged` is not a `tea --state` value and remains filtered from a single `--state all` window. `closed` uses a paginated `--state closed` walk that removes merged rows and deduplicates PR numbers in stable first-seen order until `limit` unique rows or an empty/repeated page. If the safety bound is reached first on distinct pages, the tool returns an explicit error; Gitea `open` and `all` remain single-listing paths. | `state` (string, optional — `open`/`closed`/`merged`/`all`, default `open`), `limit` (integer, optional, default 100) |
