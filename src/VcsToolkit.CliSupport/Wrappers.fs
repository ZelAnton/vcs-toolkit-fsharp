...
/// fan-out, the parse-error adapter, clone-destination cleanup, and byte-exact stdout capture.
...
module Wrappers =
    ...
    /// Run `cmd` on `core` returning **untrimmed** stdout (unlike `core.Run`, which trims the
    /// trailing newline) — for blob content and diffs where a trailing newline is significant: a
    ...
    let runUntrimmed (core: ManagedClient) (cmd: Command) : Task<Result<string, ProcessError>> =
        ...
        task {
            ...
            | Ok res ->
                ...
                | Error e -> return Error e
                | Ok ok -> return Ok(Encoding.UTF8.GetString ok.Stdout)
        }
