...
| `VcsToolkit.GitHub` | Implemented | The GitHub (`gh`) CLI client: pull requests and issues (including typed label creation and add/remove operations), Actions runs (list/view/watch/dispatch/rerun/cancel), releases, repo view, and the REST/GraphQL escape hatch — plus a `.At(dir)` cwd-bound view. Tokens are injected as `GH_TOKEN`, never in argv. |
